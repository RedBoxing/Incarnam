...
public class BinarSerialBuilder
{
    ...
    private final HashMap<Integer, ByteBufferPool> m_pools;
    private final ArrayList<Integer> m_poolSizes;
    ...
    private BinarSerialBuilder() {
        ...
        this.m_poolSizes = new ArrayList<>();
        for (int p = 1; p < 17; ++p) {
            final int size = 1 << p;
    ...
    public ByteBufferPool getPool(final int size) {
        for (int i = 0, length = this.m_poolSizes.size(); i < length; ++i) {
            final int storedSizes = this.m_poolSizes.get(i);
            if (storedSizes >= size) {
