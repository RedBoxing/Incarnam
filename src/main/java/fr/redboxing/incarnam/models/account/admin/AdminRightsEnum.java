...
public enum AdminRightsEnum {
    ...
    private final ArrayList<Short> m_moderationCommandProtocolIds;

    private AdminRightsEnum(final int id, final short[] moderationCommandProtocolIds) {
        this.m_moderationCommandProtocolIds = new ArrayList<>();
        this.m_id = (short) ((id <= -32768L) ? -32768L : ((id >= 32767L) ? 32767L : id));
        this.m_moderationCommandProtocolIds.addAll(IntStream.range(0, moderationCommandProtocolIds.length).mapToObj(s -> moderationCommandProtocolIds[s]).collect(Collectors.toList()));
    }
    ...
    public static AdminRightsEnum getRightFromCommand(final short command) {
     /*   final RightFromCommandFinder finder = new RightFromCommandFinder(command);
        for (final AdminRightsEnum adminRightsEnum : values()) {
            if (!adminRightsEnum.m_moderationCommandProtocolIds.forEach(a -> {
                final byte[] states = new byte[10];
                final short[] set = new short[10];
                int i = set.length;
                while (i-- > 0) {
                    if (states[i] == 1 && !finder.execute(set[i])) {
                        return false;
                    }
                }
                return true;
    ...
    private static class RightFromCommandFinder {
