...
public abstract class BinarSerial {
    private static final Logger LOGGER = LoggerFactory.getLogger(BinarSerial.class);
    ...
    public final void fromBuild(final byte[] binarBuild, final int version) {
        ...
        for (int i = 0; i < tocLength; ++i) {
            ...
            if (size <= 0) {
                LOGGER.warn("Part " + index + "(offset=" + offset + ") is empty for " + this + " ! (voir log serveur)", new Exception());
            }
            else if (size > 10485760) {
                LOGGER.error("Part " + index + "(offset=" + offset + ") exceeds max limit (" + size + " > " + 10485760 + " bytes)", new Exception());
            }
            else {
                ...
                if (index >= 0 && index < parts.length) {
                    ...
                    if (part == BinarSerialPart.EMPTY) {
                        LOGGER.warn("Don't know how to unserialise part #" + index + " (EMPTY).");
                        continue;
                    }
                    if (part == null) {
                        LOGGER.error("Part " + index + " of " + this + " is null", new Exception());
                        continue;
                    }
                    try {
                        part.clearError();
                    ...
                    if (partBuffer.remaining() > 0) {
                        LOGGER.warn("Part " + index + " of " + this + " still have " + partBuffer.remaining() + " byte(s) left !", new Exception());
                    }
                }
        ...
        for (int tocLength = buffer.get(), i = 0; i < tocLength; ++i) {
            ...
            if (tocIndex == index) {
                ...
                try {
                    ...
                    LOGGER.error("Exception lev\u00e9e lors de la d\u00e9serialisation de la part :" + index, e);
                }
                return;
            }
        }
